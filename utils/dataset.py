...
def split_data_from_inds_test(data, inds):
    data_reshape = {}
    client_y_list = []
    for c_i in range(len(inds)):
        x_c = []
        ...
        x_c_t = [data[i][0] for i in inds_c_t]
        y_c_t = [data[i][1] for i in inds_c_t]
        y_c_t = np.array(y_c_t)
        class_idcs = [np.argwhere(y_c_t == y).flatten()
                      for y in range(10)]
        y_list = []
        for i, c in enumerate(class_idcs):
            if len(c) != 0:
                y_list.append(i)
        client_y_list.append(y_list)
        y_c_t = list(y_c_t)
        data_reshape['client_%d' % c_i] = {'x': x_c_t, 'y': y_c_t}
...
def get_dataset(args, dataset_name, datadir, data_split_file):
    if dataset_name=='EMNIST-Letters' or dataset_name=='EMNIST-Letters-malicious' or dataset_name=='EMNIST-Letters-shuffle':
        ...
        std = (0.2752,)
        # 60000 10000
        data_train = datasets.MNIST(args.datadir, train=True, download=download, transform=transforms.Compose([
            transforms.ToTensor(), transforms.Normalize(mean, std)]))
        data_test = datasets.MNIST(args.datadir, train=False, download=download, transform=transforms.Compose([
            transforms.ToTensor(), transforms.Normalize(mean, std)]))
    ...
    if dataset_name=='TEST-noniid':
        # train_inds, test_inds, client_idcs = dirictlet_zipf_disclient(train_y_list, args.num_users)
        train_labels = np.array(train_y_list)
        test_labels = np.array(test_y_list)
        n_classes = train_labels.max() + 1
        class_idcs_train = [np.argwhere(train_labels == y).flatten()
                      for y in range(n_classes)]
        new_train_inds = [list(cli) for cli in class_idcs_train]
        class_idcs_test = [np.argwhere(test_labels == y).flatten()
                      for y in range(n_classes)]
        new_test_inds = [list(cli) for cli in class_idcs_test]
        train_inds = [[] for _ in range(args.num_users)]
        test_inds = [[] for _ in range(args.num_users)]
        for i in range(args.num_users):
            train_inds[i] = random.sample(new_train_inds[i], 200)
            test_inds[i] = random.sample(new_test_inds[i], 800)
            # train_inds[i] += random.sample(new_train_inds[8], 10)
            # test_inds[i] += random.sample(new_test_inds[8], 10)
            # train_inds[i] += random.sample(new_train_inds[9], 10)
            # test_inds[i] += random.sample(new_test_inds[9], 10)
            if i%2==0:
                train_inds[i] += random.sample(new_train_inds[i+1], 200)
                test_inds[i] += random.sample(new_test_inds[i+1], 800)
            else:
                train_inds[i] += random.sample(new_train_inds[i-1], 200)
                test_inds[i] += random.sample(new_test_inds[i-1], 800)

        data_train_reshape = split_data_from_inds_test(data_train, train_inds)
        data_test_reshape = split_data_from_inds_test(data_test, test_inds)
