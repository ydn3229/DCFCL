...
def get_data_dir(dataset):
    if 'EMnist-B' in dataset:
        path_prefix=os.path.join('data', 'EMnist', 'u5c10-cl')
    ...
    elif 'EMnist-L' in dataset:
        path_prefix=os.path.join('data', 'EMnist', 'u5c10-cl-EMNIST')
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/mnist-n10/'       

    
    elif 'Mnist-cl-5-CIFAR10' == dataset:
        path_prefix=os.path.join('data', 'Cifar10', 'u5c10-cl')
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/mnist-n10/'  
    
    elif 'cl-5' in dataset:
        path_prefix=os.path.join('data', 'Mnist', 'u5c10-cl')
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/mnist-n10/'       

    elif 'cl-1' in dataset:
        path_prefix=os.path.join('data', 'Mnist', 'u1c10-cl')
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/mnist-n10/'   

    elif 'cl-2' in dataset:
        path_prefix=os.path.join('data', 'Mnist', 'u2c10-cl')
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/mnist-n10/' 
        ...
        train_data_dir=path_prefix
        test_data_dir=path_prefix
        proxy_data_dir = path_prefix
...
def read_data(dataset):
    ...
    for f in train_files:
        ...
        train_data.update(cdata['user data'])
    ...
    return clients, groups, train_data, train_data, train_data
...
def aggregate_data_(clients, dataset, dataset_name, batch_size):
    '''
    unique_labels: labels in all datasets of all clients
    '''
    combined = []
    unique_labels = []
...
def create_generative_model(dataset, algorithm='', model='cnn', embedding=False):
    ...
    algorithm = 'FedGen' # replace the name to get the GAN in FedGen
...
def create_model(args):
    passed_dataset = get_dataset_name(args.dataset)
    model = Net(args)
    return model
